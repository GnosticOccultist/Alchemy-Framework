...
import fr.alchemy.utilities.file.io.LittleEndianInputStream;
...
/**
 * <code>FileUtils</code> provides utilities functions concerning files and
 * directories.
 * 
 * @version 0.2.0
 * @since 0.1.0
...
public final class FileUtils {
	...
	/**
	 * Return the potential extension of the file, or an empty string if null.
	 * 
	...
	/**
	 * Return the potential name of the file, or an empty string if null.
	 * 
	...
	/**
	 * Return all the files from the specified directory matching the given
	 * extension. In order to accept any extensions, use
	 * {@link FileExtensions#UNIVERSAL_EXTENSION} as the argument.
	 * 
	 * @param directory The directory to retrieve files from.
	...
	/**
	 * Add recursively all files matching the given extensions from the specified
	 * directory to the provided store. In order to accept any extensions, use
	 * {@link FileExtensions#UNIVERSAL_EXTENSION} as the argument.
	 * 
	 ...
	 * @param folders    Whether to add folders.
	 */
	public static void addFilesTo(Array<Path> store, Path directory, Array<String> extensions, boolean root,
			boolean folders) {
			...
			for (Path path : stream) {
				if (Files.isDirectory(path)) {
					...
					for (String extension : extensions) {
						if (FileExtensions.UNIVERSAL_EXTENSION.equals(extension)
								|| getExtension(path).equals(extension)) {
							store.add(path);
						}
					}
				}
	 ...
	 * @return An array of URL corresponding to each file path, or null if a
	 *         malformation has occured.
	 */
	public static URL[] toURL(Array<Path> files) {
		Validator.nonNull(files, "The array of file path can't be null!");
		URL[] urls = files.stream().map(path -> FileUtils.toURL(path)).toArray(URL[]::new);
		return urls;
	}
	 ...
	 * @return A URL corresponding to file path, or null if a malformation has
	 *         occured.
	 */
	...
	/**
	 * Open a {@link ReadableByteChannel} from the provided {@link InputStream} of a
	 * file.
	 * <p>
	...
	/**
	 * Open an {@link LittleEndianInputStream} from the provided path of a file.
	 * <p>
	 * The path cannot be null or empty.
	 * 
	 * @param path The path of the file to get an input stream.
	 * @return The little-endian input stream to the file.
	 */
	public static LittleEndianInputStream openLEStream(String path) {
		Validator.nonEmpty(path, "The path for the file cannot be null or empty!");
		return updateInternalFile(path).openLEStream();
	}
	 ...
	 * @return The little-endian input stream to the file.
	 */
	public static LittleEndianInputStream openLEStream(Path path) {
		Validator.nonNull(path, "The path for the file cannot be null!");
		InputStream is = null;
		try {
			is = Files.newInputStream(path);
	...
	/**
	 * Open a {@link ProgressInputStream} from the provided path of a file using the
	 * given {@link ProgressListener} to keep track of bytes being read.
	 * <p>
	 * The path cannot be null or empty.
	 * 
	 * @param path     The path of the file to get an input stream.
	...
	/**
	 * Converts the provided file into the provided {@link ByteBuffer}. If the
	 * buffer is too small, it will execute the given resizing action.
	 * 
	 ...
	 */
	public static ByteBuffer toByteBuffer(String resource, ByteBuffer buffer,
			BiModifierAction<ByteBuffer, Integer> resizeAction) {
	 ...
	 * @return The filled byte buffer.
	 */
	public static ByteBuffer toByteBuffer(Path path, ByteBuffer buffer,
			BiModifierAction<ByteBuffer, Integer> resizeAction) {
		return toByteBuffer(openStream(path), buffer, resizeAction);
	}

	/**
	 * Converts the provided {@link InputStream} into the provided
	 * {@link ByteBuffer}. If the buffer is too small, it will execute the given
	 * resizing action.
	 * 
	 ...
	 * @return The filled byte buffer.
	 */
	public static ByteBuffer toByteBuffer(InputStream is, ByteBuffer buffer,
			BiModifierAction<ByteBuffer, Integer> resizeAction) {
	...
	/**
	 * Loads and return the {@link Properties} from the provided path using the
	 * given store, or a new instance if null.
	 * 
	 * @param path       The path of the property file to load (not null, not
	 *                   empty).
	...
	/**
	 * Loads and return the {@link Properties} from the provided {@link Path} using
	 * the given store, or a new instance if null.
	 * 
	 * @param path       The path of the property file to load (not null, not
	 *                   empty).
	 * @param properties The properties to populate with the loaded data.
	...
	/**
	 * Creates all directories contained in the {@link Path} if they don't already
	 * exist.
	 * 
	...
	/**
	 * Creates a file contained in the given {@link Path} if it doesn't already
	 * exist.
	 * 
	...
	/**
	 * Closing safely the given {@link Closeable} implementation without needing to
	 * catch potentially thrown {@link IOException}.
	 * 
	...
	/**
	 * Performing safely the provided {@link SafeVoidAction} using the given object,
	 * without needing to catch potentially thrown {@link Throwable}.
	 * 
	...
	/**
	 * Performing safely the provided {@link SafeModifierAction} using the given
	 * object, without needing to catch potentially thrown {@link Throwable}.
	 * 
	 * @param object     The object to perform an action safely with (not null).
	 * @param safeAction The safe action to perform with the object (not null).
	 * @return The returned object for chaining purposes (not null).
