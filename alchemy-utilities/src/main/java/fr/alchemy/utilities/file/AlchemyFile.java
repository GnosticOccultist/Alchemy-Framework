...
/**
 * <code>AlchemyFile</code> is a more complex implementation of {@link File} to
 * manage {@link InputStream} or {@link OutputStream}.
 * 
 * @version 0.2.0
 * @since 0.1.0
 * 
...
public class AlchemyFile {
	...
	/**
	 * Instantiates a new <code>AlchemyFile</code> with an empty path. This
	 * constructor should only be accessed by {@link FileUtils} to instantiate a
	 * local variable.
	 */
	...
	/**
	 * Change the path of the <code>AlchemyFile</code>, only virtually, to the
	 * provided one, updating its extension as well, and returns the previous path
	 * used by the file.
	 * 
	...
	/**
	 * Return the folder path of the <code>AlchemyFile</code> or an empty path if
	...
	/**
	 * Acquire the internal URL of the <code>AlchemyFile</code>, which can be used
	 * to obtain an {@link InputStream}.
	 * 
	...
	/**
	 * Open an {@link InputStream} using the URL of the <code>AlchemyFile</code>
	 * obtained by {@link #acquireInternalURL()}.
	 * 
	 * @return The input stream of the file, or null if no file with this path was
	 *         found.
	 */
	...
	/**
	 * Open an {@link LittleEndianInputStream} using the URL of the
	 * <code>AlchemyFile</code> obtained by {@link #acquireInternalURL()}.
	 * 
	 * @return The input stream of the file, or null if no file with this path was
	 *         found.
	 */
	public LittleEndianInputStream openLEStream() {
		return new LittleEndianInputStream(openStream());
	}

	/**
	 * Open a {@link ProgressInputStream} using the URL of the
	 * <code>AlchemyFile</code> obtained by {@link #acquireInternalURL()}.
	 * 
	 * @return The progress input stream of the file, or null if no file with this
	 *         path was found.
	 */
	...
	/**
	 * Open a {@link ProgressInputStream} using the URL of the
	 * <code>AlchemyFile</code> obtained by {@link #acquireInternalURL()}, and the
	 * given {@link ProgressListener} to keep track of bytes read.
	 * 
	...
	/**
	 * Return the <code>AlchemyFile</code> as a {@link Path} instance using a
	 * corresponding {@link URI}.
	 * 
	 * @return A path instance representing the file, or null if no file with this
	 *         path was found.
	 */
	public Path asPath() {
		try {
