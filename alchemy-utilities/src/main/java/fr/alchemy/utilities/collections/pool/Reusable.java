 ...
 * <p>
 * The main usage of this interface reside in a pool where it can be retrieved and re-injected when needed for 
...
public interface Reusable extends AutoCloseable {
	...
	/**
	 * Cleanup the <code>Reusable</code> before for example injecting it back to a
	 * pool.
	 */
	default void free() {}

	/**
	 * Prepares the <code>Reusable</code> to be reused when for example retrieved
	 * from a pool.
	 */
	default void reuse() {}
	...
	/**
	 * Close and release the <code>Reusable</code> by injecting it back to its
	 * original pool.
	 * 
	 * @see #release()
	 */
