...
/**
 * <code>FactoryLogger</code> is a utility class to manage and store each
 * instantiated {@link Logger}.
 * <p>
 * By default the factory uses the {@link AlchemyLogger} as its base
 * implementation but you can change it by calling {@link #changeToType(Class)}
 * to your own logger implementation if you have designed one.
 * <p>
 * A logger can be registered or retrieved at any time using
 * {@link #getLogger(Class)} or {@link #getLogger(String)}, and then be used to
 * write something in the console using the printing function, by default this
 * is <code>System.err::println</code> but this can be changed with
 * {@link #changePrinter(Consumer)}.
 * 
 * @version 0.2.1
 * @since 0.1.0
 * 
...
public final class FactoryLogger {
	 ...
	 */
	private static Class<? extends Logger> loggerType = AlchemyLogger.class;
	/**
	 * The function to print the message (default&rarr;System.err::println).
	 */
	private static Consumer<String> printer = System.err::println;
	 ...
	 * <p>
	 * Invoking this method will result in destroying all previously created logger
	 * if the implementation to use differs from the old one.
	 * 
	...
	/**
	 * Writes the provided message at the specified {@link LoggerLevel} and with the
	 * given name using the registered printer (by default:
	 ...
	 */
	public static void print(LoggerLevel level, String name, CharSequence message) {
	...
	/**
	 * Writes the provided message at the specified {@link LoggerLevel} using the
	 * registered printer (by default: <code>System.err.println</code>).
	 * 
	...
	/**
	 * Returns the {@link Logger} for the provided class or instantiate a new one if
	...
	/**
	 * Returns the {@link Logger} with the provided name or instantiate a new one if
	 * it doesn't exist yet.
	 * 
