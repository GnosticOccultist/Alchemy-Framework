...
/**
 * <code>AlchemyLogger</code> is an implementation of {@link Logger} for the
 * <code>Alchemy-Framework</code>, and can be reused over multiple projects.
 * 
 * @version 0.2.1
 * @since 0.1.0
 * 
...
public class AlchemyLogger implements Logger {
	 ...
	 * Please use {@link FactoryLogger#getLogger(String)} to create a new logger
	 * instance.
	 */
	 ...
	 */
	@Override
	public void print(LoggerLevel level, CharSequence message) {
		if (isActive(level)) {
	 ...
	 */
	@Override
	public void print(LoggerLevel level, CharSequence message, Throwable exception) {
		if (isActive(level)) {
			StringWriter writer = new StringWriter();
			PrintWriter printWriter = new PrintWriter(writer);
			exception.printStackTrace(printWriter);
